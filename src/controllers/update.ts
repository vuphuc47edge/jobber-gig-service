...
import { BadRequestError, ISellerGig, isDataURL, upload } from '@vuphuc47edge/jobber-shared';
...
export const gigUpdate = async (req: Request, res: Response): Promise<void> => {
  ...
  if (isDataUrl) {
    const result: UploadApiResponse = (await upload(req.body.coverImage)) as UploadApiResponse;
...
export const gigUpdateActive = async (req: Request, res: Response): Promise<void> => {
